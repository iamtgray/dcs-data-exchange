...
```
Lab 1:  Data in S3 with labels. Anyone can read it.

Lab 2:  Lambda checks Verified Permissions before returning data.
        But the data in S3 is still plaintext.

Lab 3:  Data is encrypted locally as a TDF file before upload.
        To decrypt, the CLI contacts the KAS.
        KAS checks your Cognito token against the TDF's policy.
        Only then does KAS ask KMS to unwrap the encryption key.
        Even S3 admins see only ciphertext.
```
