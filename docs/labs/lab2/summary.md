...
The biggest gap: S3 stores all data in plain text. Anyone with direct S3 access (an admin, AWS root account, a compromised IAM credential) can read everything. The ABAC policies are only enforced by our Lambda, not at the storage layer.
