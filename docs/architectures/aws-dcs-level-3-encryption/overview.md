...
This architecture implements **DCS Level 3 (Protection via Encryption)**, the most complete DCS level. It deploys the OpenTDF platform on AWS to provide cryptographic data protection where security travels with the data itself, regardless of where it's stored or transmitted.
...
This architecture deliberately uses the simplest possible AWS setup so you can focus on DCS concepts rather than networking:
...
## The difference: why Level 3 matters
