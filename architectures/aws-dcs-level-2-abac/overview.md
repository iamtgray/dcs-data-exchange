...
```
+------------------------------------------------------------------+
|                     "Coalition HQ" Account                        |
|                                                                   |
|  +---------------------------+                                    |
|  | Amazon Verified           |  Cedar Policy Store:               |
|  | Permissions               |                                    |
|  | (Policy Decision Point)   |  permit(principal, action, resource)|
|  |                           |  when {                            |
|  | Evaluates:                |    principal.clearance >= resource. |
|  | - User clearance          |      classification                |
|  | - User nationality        |    && principal.nationality in     |
|  | - User SAPs               |      resource.releasableTo         |
|  | - Data classification     |    && (resource.sap == "NONE"      |
|  | - Data releasability      |      || principal.sap.contains(    |
|  | - Data SAP requirements   |        resource.sap))              |
|  |                           |  };                                |
|  +------------+--------------+                                    |
|               |                                                   |
|               v                                                   |
|  +----------------------------------------------------------+   |
|  |                   API Gateway                              |   |
|  |  POST /authorize  - Check if access should be granted      |   |
|  |  GET  /data/{id}  - Retrieve data (after authorization)    |   |
|  |  POST /data       - Store data (with label validation)     |   |
|  +---------------------------+--------------------------------+   |
|                              |                                    |
|         +-------------------++-------------------+                |
|         v                                        v                |
|  +----------------+                    +-------------------+      |
|  | Lambda          |                    | DynamoDB          |     |
|  | Data Service    |                    | Data Store        |     |
|  |                 |                    |                   |      |
|  | - Calls AVP for |                    | Table: dcs-data   |     |
|  |   authorization |                    | PK: data-id       |     |
|  | - Retrieves data|                    | Attrs:            |     |
|  |   if authorized |                    |  classification   |     |
|  | - Stores data   |                    |  releasableTo     |     |
|  |   with labels   |                    |  sap              |     |
|  | - Logs decisions|                    |  originator       |     |
|  +----------------+                    |  payload          |      |
|                                        |  created          |      |
|                                        +-------------------+      |
|                                                                   |
+------------------------------------------------------------------+
         ...
         v                    v                    v
+----------------+  +----------------+  +----------------+
| "UK" Account   |  | "Poland"       |  | "US" Account   |
|                |  | Account        |  |                |
| Cognito User   |  | Cognito User   |  | Cognito User   |
| Pool           |  | Pool           |  | Pool           |
|                |  |                |  |                |
| Users with:    |  | Users with:    |  | Users with:    |
| - clearance:   |  | - clearance:   |  | - clearance:   |
|   SECRET       |  |   NATO-SECRET  |  |   IL-6         |
| - nationality: |  | - nationality: |  | - nationality: |
|   GBR          |  |   POL          |  |   USA          |
| - sap: [WALL]  |  | - sap: []      |  | - sap: [WALL]  |
+----------------+  +----------------+  +----------------+
```
