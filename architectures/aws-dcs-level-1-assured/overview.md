...
|  | (SECRET   |     | (NATO     |     | (UNCLASSIFIED            |      |
|  |  GBR)     |     |  SECRET)  |     |  contractor)             |      |
|  +-----+-----+     +-----+-----+     +----------+---------------+      |
|        |                 |                       |                      |
|        v                 v                       v                      |
|  +----------------------------------------------------------------+    |
...
|  +----------------------------------------------------------------+    |
|  |                Lambda Authorizer                                |    |
|  |  1. Reads user attributes (clearance, nationality, SAPs)       |    |
|  |  2. Reads STANAG 4774 label from DynamoDB                      |    |
|  |  3. Verifies STANAG 4778 signature via KMS                     |    |
|  |  4. Evaluates policy: user attrs vs label requirements         |    |
|  |  5. Returns allow/deny with full audit context                 |    |
|  +----------------------------------------------------------------+    |
|           |                    |                    |                    |
|           v                    v                    v                    |
|  +----------------+  +-----------------+  +---------------------+      |
|  | S3 Data Bucket |  | DynamoDB Label  |  | KMS Asymmetric Key  |      |
|  |                |  | Store           |  | (RSA_2048 SIGN)     |      |
|  | intel-001.pdf  |  |                 |  |                     |      |
|  | sitrep-042.json|  | PK: object_key  |  | - Signs labels      |      |
|  | logistics.csv  |  | label_xml: ...  |  | - Private key in HSM|      |
|  |                |  | signature: ...  |  | - Public key for    |      |
|  | (raw data,     |  | data_hash: ...  |  |   verification      |      |
|  |  no labels)    |  | signed_at: ...  |  +---------------------+      |
|  +----------------+  +-----------------+                                |
|                                                                         |
|  +----------------------------------------------------------------+    |
|  |                 Lambda Label Service                            |    |
|  |  - Creates STANAG 4774 XML labels                              |    |
|  |  - Computes SHA-256 hash of S3 object                          |    |
|  |  - Signs label+hash binding via KMS (STANAG 4778)              |    |
|  |  - Stores signed label in DynamoDB                             |    |
|  +----------------------------------------------------------------+    |
|                                                                         |
|  +----------------------------------------------------------------+    |
|  |                    CloudTrail                                   |    |
|  |  - Logs every S3 access, DynamoDB read, KMS sign/verify        |    |
|  |  - Immutable audit trail in separate S3 bucket                 |    |
|  +----------------------------------------------------------------+    |
+------------------------------------------------------------------------+
```
...
```
+-------------------+     +-------------------+
| S3 Object         |     | STANAG 4774 Label |
| (intel-001.pdf)   |     | (XML document)    |
+--------+----------+     +--------+----------+
         |                          |
         v                          v
   SHA-256 Hash              Canonical XML
   of object content         (C14N serialization)
         |                          |
         +----------+---------------+
                    |
                    v
