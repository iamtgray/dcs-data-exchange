...
This architecture implements **DCS Level 1 (Control/Labeling)** using native AWS services. It shows how security metadata can be attached to data objects and used to make access decisions, which is the foundation of all data-centric security.
...
|  +----------------------------------------------------------+   |
|  |                   Lambda Authorizer                        |   |
|  |  - Reads user attributes (clearance, nationality, SAPs)   |   |
|  |  - Reads object labels (S3 tags)                          |   |
|  |  - Compares user attrs vs data labels                     |   |
|  |  - Returns allow/deny                                     |   |
|  +----------------------------------------------------------+   |
|                              |                                    |
|            +----------------+--+----------------+                 |
...
|  | intel-001.pdf  |  | sitrep-042.json|  | logistics.csv  |     |
|  |                |  |                |  |                |      |
|  | Tags:          |  | Tags:          |  | Tags:          |     |
|  | classification:|  | classification:|  | classification:|     |
|  |   SECRET       |  |   NATO-SECRET  |  |   UNCLASSIFIED |     |
|  | releasable-to: |  | releasable-to: |  | releasable-to: |     |
|  |   GBR,USA      |  |   GBR,USA,POL  |  |   ALL          |     |
...
|  +----------------------------------------------------------+   |
|  |                    CloudTrail                              |   |
|  |  - Logs every S3 access attempt                           |   |
|  |  - Logs API Gateway authorization decisions                |   |
|  |  - Logs Lambda invocations                                 |   |
|  |  - Immutable audit trail in separate S3 bucket             |   |
|  +----------------------------------------------------------+   |
|                                                                   |
|  +----------------------------------------------------------+   |
|  |                    Lambda Auto-Labeler                     |   |
|  |  - Triggered on S3 PutObject                              |   |
|  |  - Analyzes content for classification indicators          |   |
|  |  - Applies S3 tags based on content analysis               |   |
|  |  - Logs labeling decisions                                 |   |
|  +----------------------------------------------------------+   |
+------------------------------------------------------------------+
```
